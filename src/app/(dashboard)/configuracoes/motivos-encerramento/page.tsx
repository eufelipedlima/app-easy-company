...
export default function MotivosEncerramentoPage() {
  ...
  async function adicionar(e: React.FormEvent) {
    ...
    setSalvando(true);
    const supabase = createClient();
    await supabase.from("motivos_encerramento").insert({ nome: novoNome.trim() });
    setNovoNome("");
    setSalvando(false);
    carregar();
  }
  ...
  async function remover(id: string) {
    const supabase = createClient();
    await supabase.from("motivos_encerramento").delete().eq("id", id);
    carregar();
  }
    ...
    <section>
      <form onSubmit={adicionar} className="flex gap-2 mb-4">
        <input
          ...
          className="input"
          placeholder="Ex: Insatisfação com o serviço..."
        />
        <button
          type="submit"
          disabled={salvando}
      ...
      <div className="rounded-3xl bg-card border border-black/5 overflow-hidden">
        {loading ? (
          ...
          <p className="p-4 text-sm text-ink/50">Nenhum motivo cadastrado ainda.</p>
        ) : (
          itens.map((item) => (
            <div
              key={item.id}
