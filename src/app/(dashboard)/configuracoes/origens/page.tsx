...
export default function OrigensPage() {
  ...
  async function adicionar(e: React.FormEvent) {
    ...
    setSalvando(true);
    const supabase = createClient();
    await supabase.from("origens").insert({ nome: novoNome.trim() });
    setNovoNome("");
    setSalvando(false);
    carregar();
  }
    ...
    <section>
      <form onSubmit={adicionar} className="flex gap-2 mb-4">
        <input
          ...
          className="input"
          placeholder="Ex: Anúncio, Evento, Indicação..."
        />
        <button
          type="submit"
          disabled={salvando}
      ...
      <div className="rounded-3xl bg-card border border-black/5 overflow-hidden">
        {loading ? (
          ...
          <p className="p-4 text-sm text-ink/50">Nenhuma origem cadastrada ainda.</p>
        ) : (
          itens.map((item) => (
            <div
              key={item.id}
