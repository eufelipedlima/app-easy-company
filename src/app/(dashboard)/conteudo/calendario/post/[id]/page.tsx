...
export default function PostDetalhePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function iniciarCronometro() {
    ...
    // tela já tinha na memória. Se já tiver uma sessão rodando (nessa aba,
    // em outra aba, ou em outro dispositivo), não sobrescreve a hora de
    // início — só sincroniza a tela com o que já está rodando de verdade.
    ...
    await supabase
      .from("posts_conteudo_tempo_sessoes")
      .upsert(
        { post_id: id, funcionario_auth_id: meuId, iniciado_em: agora, segundos_acumulados: acumuladoAtual },
        { onConflict: "post_id,funcionario_auth_id" }
      );
    setSessoesTempo((atual) => {
      const semEu = atual.filter((s) => s.funcionario_auth_id !== meuId);
  ...
  async function pausarCronometro() {
    ...
    const { data: postAtual } = await supabase.from("posts_conteudo").select("tempo_total_segundos").eq("id", id).maybeSingle();
    const novoTotalGeral = (postAtual?.tempo_total_segundos ?? post.tempo_total_segundos) + segundosCorridos;
    await Promise.all([
      supabase
        .from("posts_conteudo_tempo_sessoes")
            ...
            {abaLateral === "comentarios" ? (
              ...
              <div className="flex-1 overflow-y-auto px-5 py-4">
                ...
                {(() => {
                  ...
                  const naoIdentificado = post.tempo_total_segundos - somaIdentificada;
                  return (
                    <div className="space-y-3">
                      ...
                      <div className="flex items-center justify-between text-sm pt-3 border-t border-black/5">
                        ...
                        <span className="font-bold text-ink">{formatarDuracaoLonga(post.tempo_total_segundos)}</span>
                      </div>
                    </div>
                  );
                })()}
