  ...
  async function enviarComentario(arquivos: File[]) {
    ...
    try {
      ...
      for (const arquivo of arquivos) {
        ...
        const { error: erroAnexo } = await supabase.from("tarefas_comentarios_anexos").insert({
      ...
      if (mencionados.length > 0) {
        await supabase.from("notificacoes").insert(
          mencionados.map((c) => ({
            ...
            tipo: "mencao_tarefa",
            titulo: `${meuNome} te mencionou numa tarefa`,
            descricao: tarefa?.titulo ?? "Novo comentário",
            link: `/tarefas/${id}`,
            autor_id: meuId,
            autor_nome: meuNome,
            autor_foto_url: meuFotoUrl,
        ...
        <div className="flex-1 overflow-y-auto px-8 py-6 max-w-3xl mx-auto w-full">
          ...
          <div className="rounded-2xl bg-white p-4 shadow-sm mb-6 space-y-3">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <div>
                <span className="block text-[10px] font-bold uppercase tracking-wide text-ink/40 mb-1">Prioridade</span>
                <select
                  value={prioridade}
                  onChange={(e) => {
