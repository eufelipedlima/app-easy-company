...
//
// Nomes de coluna conferidos:
// - lancamentos: despesa_fixa_id, data_vencimento, data_competencia, descricao, valor
// - despesas_fixas: nome, valor_mensal, banco_id, plano_conta_id (conferidos via
//   o insert em lancamento-form.tsx). O nome da coluna de status/ativo abaixo
//   ("status" = "ativo") NÃO foi conferido — ajuste se o campo real for diferente
//   (ex: um boolean "ativo").
...
export async function conferirRollingDespesasFixas() {
  ...
  const { data: despesasFixas, error } = await supabase
    .from("despesas_fixas")
    .select("id, nome, valor_mensal, banco_id, plano_conta_id, ultima_verificacao_parcelas")
    .eq("status", "ativo");
  ...
  for (const despesa of (despesasFixas ?? []) as DespesaFixa[]) {
    ...
    if (parcelasRestantes <= LIMITE_MINIMO) {
      await gerarParcelasFaltantes(supabase, despesa, parcelasRestantes);
    }
...
async function gerarParcelasFaltantes(
  ...
  const baseData = ultima ? new Date(ultima.data_vencimento + "T00:00:00") : new Date();
